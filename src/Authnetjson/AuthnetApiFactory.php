...
class AuthnetApiFactory
{
     ...
     */
    public static function getJsonApiHandler($login, $transaction_key, $endpoint = null)
    {
        $login = trim($login);
        $transaction_key = trim($transaction_key);
        $endpoint = isset($endpoint) ? $endpoint : self::USE_CDN_SERVER;
     ...
     * @throws Exception
     */
    public static function getSimHandler($login, $transaction_key, $server = null)
    {
        $login = trim($login);
        $transaction_key = trim($transaction_key);
        $server = isset($server) ? $server : self::USE_PRODUCTION_SERVER;
     ...
     * @throws AuthnetInvalidServerException
     */
    public static function getWebhooksHandler($login, $transaction_key, $server = null)
    {
        $login = trim($login);
        $transaction_key = trim($transaction_key);
        $server = isset($server) ? $server : self::USE_PRODUCTION_SERVER;
